...
from repositories.models import Country

LOC_COUNTRIES_URL = "http://id.loc.gov/vocabulary/countries.json"
MADS_CODE_KEY = "http://www.loc.gov/mads/rdf/v1#code"
MADS_LABEL_KEY = "http://www.loc.gov/mads/rdf/v1#authoritativeLabel"


class Command(BaseCommand):
    help = "Load ISO 3166-1 alpha-2 country codes from Library of Congress MARC Countries vocabulary"

    def add_arguments(self, parser):
        parser.add_argument(
            "--url",
            default=LOC_COUNTRIES_URL,
            help="URL to fetch MARC Countries vocabulary JSON",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="Include 3-character codes for US states and Canadian provinces (default: only 2-character ISO 3166-1 codes)",
        )

    def handle(self, *args, **options):
        url = options["url"]
        dry_run = options["dry_run"]
        include_subdivisions = options["include_subdivisions"]

        self.stdout.write(f"Fetching MARC Countries vocabulary from {url}...")
        ...
        for entry in data:
            ...
            # Filter to only 2-character codes (ISO 3166-1 alpha-2) unless --include-subdivisions is set
            if not include_subdivisions and len(code) != 2:
                skipped_count += 1
                continue
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Dry run: would create/update {created_count} countries, "
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Loaded ISO 3166-1 countries: {created_count} created, "
